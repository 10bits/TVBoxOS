...
import io.zwz.analyze.help.JsExtensions
...
class AnalyzeRule() : JsExtensions {
    ...
    init {
        setVariableData(ruleData)
    }

    @JvmOverloads
    fun setContent(content: Any?, baseUrl: String? = null, ruleData: RuleDataInterface? = null): AnalyzeRule {
    ...
    private fun setVariableData(ruleData: RuleDataInterface?): AnalyzeRule {
        if (ruleData == null) {
    ...
    fun getStringList(ruleList: List<SourceRule>, isUrl: Boolean = false): List<String>? {
        ...
        } else if (result is ConsString) {
            result = (result as ConsString).split("\n")
        }
        if (isUrl) {
